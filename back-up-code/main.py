...
from classes.services import Service_Class,Service_Instance,DataFormatObject
import os
import subprocess
import sys
import json
...
try:
    ...
    print "Method cound be : back-forward, back-tracking"
    sys.exit(0)
#method = "BACKTRACKING"
...
ASP_RULE_GOAL_DATA_FORMAT = "goal_data_format(T,Time) :- exists_data_format(%s,T,Time),consider_abtract_step(T)."
...
def solve_abtract_level(number_of_model,path_to_planning_base,path_to_ontology_base,path_to_initial,path_to_goal):
    clingo_command = [clingo_path, '--outf=2', '-n',str(number_of_model),path_to_planning_base,path_to_ontology_base,path_to_initial,path_to_goal]
...
def single_run_planning_mapping_data_format(list_input_data_format, list_output_data_format, abtract_step, index_model):
    ...
    #if ("newickTree" in list_input_data_format):
...
def read_data_from_abstract_planning(consider_planning_data_json):
    ...
    '''
    print "----Goal----"
    print LIST_OF_GOAL_ABTRACT_PREDICATE
    print "----List of abtract level-----"
    print LIST_OF_OCCUR_PREDICATE
    print "----List of possible concrete-----"
    print LIST_OF_POSSIBLE_CONCRETE_OP_INSTANCES_PREDICATE
    '''
...
def procecure_abtract_planning():
    planning_data = solve_abtract_level(1,os.path.join(FULL_PATH_ABTRACT_PLANNING_ENGINE_MODEL,configuration.CONFIGURATION_DATA['abtract_level_planning_asp_file']),os.path.join(FULL_PATH_ABTRACT_PLANNING_ENGINE_MODEL,configuration.CONFIGURATION_DATA['ontology_web_serivce_asp']),os.path.join(FULL_PATH_PLANNING_STATES_FOLDER,configuration.CONFIGURATION_DATA['initial_state_asp_file']),os.path.join(FULL_PATH_PLANNING_STATES_FOLDER,configuration.CONFIGURATION_DATA['goal_state_asp_file']))
...
def procedure_concrete_planning():
    ...
    GLOBAL_ABTRACT_PLANNING.print_workflow()
...
#print procedure_concrete_planning() 
def main():
